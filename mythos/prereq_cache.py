"""Persist reverse-prerequisite trees across Mythos process restarts.

The retired Hermes MCP kept ``PREREQUISITE_CACHE`` as a process-local dict, so
every server restart re-derived the same trees. The live equivalent is the
cartographer knowledge map. This sidecar stores extracted prerequisite names
